...
def explore(node, depth=1, explode: nil, &blk)
  if explode == nil
    explore(node, depth, explode: true, &blk)
    explore(node, depth, explode: false, &blk)
    return
  end

  if depth >= 4 && !node.v && explode
  ...
  if node.l
    explore(node.l, depth+1, explode: explode, &blk)
    explore(node.r, depth+1, explode: explode, &blk)
  elsif node.v >= 10 && !explode
    # split
